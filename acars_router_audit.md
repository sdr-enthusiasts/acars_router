...
| PR12: `log` → `tracing` migration + `RUST_LOG` env-filter; drop `sdre-rust-logging`           | §4.6                                                                         | **Done** | `42194ce`     |
| PR12b: per-task `#[instrument]` spans on every pipeline entry point                           | §4.6 follow-up                                                               | **Done** | `51503c9`     |

**Open strategic question:** ~~`log` vs `tracing` (audit §4.6). `tokio` already has the `tracing` feature enabled but the codebase uses `log` via `sdre-rust-logging` (which is `env_logger`-based). Switching is a workspace-wide change touching every log macro and would drop `sdre-rust-logging`. Decision deferred pending owner input.~~ Resolved in PR12: workspace migrated to `tracing` + `tracing-subscriber` with `EnvFilter`. `RUST_LOG` honoured with full env-filter syntax (per-crate/per-module levels); `AR_VERBOSITY` retained as fallback when `RUST_LOG` is unset. `sdre-rust-logging` dropped from the dep tree.

**Remaining open items:**

- ~~`log` vs `tracing` decision (§4.6).~~ Done in PR12.
- PR12 only swapped the macros. Per-task `#[instrument]` spans (`protocol`, `transport`, `peer` fields) replacing the literal `[Message Handler ACARS]`-style prefixes baked into ~50 log strings is partially addressed by PR12b: spans are now attached to every pipeline task entry point. The literal `[ACARS]`/`[TCP SERVER X]`-style string prefixes inside individual log calls are retained for now; mechanical strip of the ~82 prefixed log strings is left as a future PR12c once the new span output is validated against real deployments.
