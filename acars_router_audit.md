...
| PR12c: mechanical strip of 82 `[X {}] ` log prefixes now duplicated by spans                  | §4.6 follow-up                                                               | **Done** | `ab93879`     |
...
- ~~PR12 only swapped the macros.~~ Spans landed in PR12b; the 82 literal `[ACARS]`/`[TCP SERVER X]`-style string prefixes were stripped in PR12c (`ab93879`). Two helper params (`clean_up_dedupe_cache::queue_type`, `forward_to_peer::proto`) that existed only to feed the stripped prefixes were also removed.
