        ...
        match self {
            Some(reassembled_msg) => {
                ...
                match parsed_msg {
                    ...
                    Ok(msg) => {
                        trace!("[{} Listener SERVER: {}] Received message: {:?}", listener_type, proto_name, msg);
                        match channel.send(msg).await {
                            Ok(_) => debug!("[{} Listener SERVER: {}] Message sent to channel", listener_type, proto_name),
                            Err(e) => error!("[{} Listener SERVER: {}] sending message to channel: {}", listener_type, proto_name, e),
                        };
                    }
                }
            }
            None => trace!("[{} Listener SERVER: {}] Invalid Message", listener_type, proto_name),
        }
    }
}
...
impl PacketHandler {
    ...
    pub async fn attempt_message_reassembly(&self, new_message_string: String, peer: SocketAddr) -> Option<AcarsVdlm2Message> {
        ...
        if self.queue.lock().await.contains_key(&peer) {
            info!("[UDP SERVER: {}] Message received from {} is being reassembled", self.name, peer);
            ...
            match message_for_peer.decode_message() {
                Err(e) => info!("{e}"),
                Ok(msg_deserialized) => {
                    info!("[UDP SERVER: {}] Reassembled a message from peer {}", self.name, peer);
        ...
        match output_message {
            ...
            None => {
                ...
                self.queue.lock().await.insert(peer, (message_queue_time, message_for_peer));
            }
        }
